    ...
    while True:
        try:
            with open(f"Pet{nome}.txt", "r", encoding="UTF-8") as file:
                ...
                novo_tipo = input(
                    "Novo tipo (pressione Enter caso não deseje trocar): ").strip().capitalize()
                novo_tipo = novo_tipo if novo_tipo else linhas[2].split(":")[
                    1].strip()
...
def editar_pet():
    ...
    try:
        ...
        novo_nome = input(
            "Novo nome (pressione Enter caso não deseje trocar): ").strip().capitalize()
        novo_nome = novo_nome if novo_nome else linhas[0].split(":")[1].strip()

        nova_idade = input(
            "Nova idade (pressione Enter caso não deseje trocar): ").strip()
        try:
            nova_idade = int(nova_idade) if nova_idade else int(
                linhas[1].strip().split(":")[1].strip())
        except ValueError:
